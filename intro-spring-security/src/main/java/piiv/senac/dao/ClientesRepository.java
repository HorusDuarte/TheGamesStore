...
public void aaaClientes(table_Cliente u) {
    ...
    try {
        stmt = con.prepareStatement("insert into table_Cliente (nome_completo, email_login,cpf,endereco,cep,usuario,senha) values (?, ?, ?,?,?,?,?);");

        stmt.setString(1, u.getNome_completo());
        stmt.setString(2, u.getEmail_login());
...
public List<table_Cliente> getTable_Cliente() {
    ...
    try {
      ...
      while (rs.next()) {
    	 ...
    	 p.setEmail_login(rs.getString("email_login"));
