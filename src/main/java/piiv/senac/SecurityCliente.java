	...
	protected void configure(AuthenticationManagerBuilder auth) throws Exception{
		...
		auth.jdbcAuthentication().dataSource(dataSource)
			.usersByUsernameQuery(
					"select usuario as username, senha as password, 1 as enable from table_Cliente where usuario=?")
			.authoritiesByUsernameQuery(
					"select usuario as username, 'cliente' as authority from table_Cliente where usuario=?")
